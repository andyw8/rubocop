...
Over time, entries in `.rubocop_todo.yml` can become redundant, e.g. when an
offense is fixed but its `Exclude` entry is left behind. Run `rubocop
--check-todo` to detect this without modifying the file; it exits with an
error and lists the cops with redundant entries, which is useful as a CI
check.
